  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/receipts', {
      ...
      body: JSON.stringify({
        ...
        image_source: imagePath ? 'upload' : null,
      }),
    })
    ...
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        ...
        <div>
          ...
          <input type="text" name="payee_name" value={form.payee_name} onChange={handleChange}
            placeholder={t('payeeNamePlaceholder')} required
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        ...
        <div>
          ...
          <input type="text" name="payee_address" value={form.payee_address} onChange={handleChange}
            placeholder={t('payeeAddressPlaceholder')}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className="relative">
              <input type="number" name="tax_amount" value={form.tax_amount} onChange={handleChange}
                placeholder="0" min="0"
                className="w-full border border-gray-300 rounded-lg px-3 py-2 pr-8 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              <span className="absolute right-3 top-2 text-sm text-gray-400">{t('currencyUnit')}</span>
            </div>
        ...
        <div>
          ...
          <textarea name="memo" value={form.memo} onChange={handleChange}
            placeholder={t('memoPlaceholder')} rows={3}
